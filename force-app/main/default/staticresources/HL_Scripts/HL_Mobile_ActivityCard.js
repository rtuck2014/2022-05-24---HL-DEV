function loadContactActivities() {
    if(contactId)
    {
        var query = "SELECT WhatId, WhoId, StartDateTime, EndDateTime, Start_Date__c, Start_Time__c, End_Date__c, End_Time__c, Sync_To_Outlook__c, Notify_Coverage_Team__c, Subject, Description, Type, ParentId__c, Private__c, Primary_Attendee__c, Primary_External_Contact__c, CreatedById, CreatedBy.Name FROM EVENT WHERE WhoId = '" +  contactId + "' AND ParentId__c <> null AND (EndDateTime > TODAY OR EndDateTime = LAST_N_DAYS:730) ORDER BY EndDateTime DESC";
