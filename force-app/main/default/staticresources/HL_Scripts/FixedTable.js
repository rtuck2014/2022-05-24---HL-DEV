/*
ZENGMAN - 08/31/2015 - Modified to work with Salesforce ($j)
...
(function ($j) {
	...
	$j.fn.fxdHdrCol = function (o) {
		...
		return this.each (function () {
			...
			var $ths = $j('thead tr', $this).first().find('th');
			...
			$ths.each(function (i, el) {
				...
				$j(el).css({width: calcWidth, textAlign: cfg.colModal[ct-1].align});
				
				lc.tableWidth += calcWidth + thSpace + ((i == 0)?2:1);
			});
								
			$j('tbody', $this).find('tr').each(function (i, el) {
				$j('td', el).each(function (i, tdel) {
			...
			var theadTr = $j('thead', $this);
			//clone the thead->tr 
			var theadTrClone = theadTr.clone();
			
			//construct fixed row (full row)
			lc.ft_rel_container
				.prepend($j(cfg.tableTmpl(), {'class': 'ft_r ui-widget-header'})
				.append(theadTrClone));

			//an instance of fixed row
			lc.ft_r = $j('.ft_r', lc.ft_rel_container);
			lc.ft_r.wrap($j('<div />', {'class': 'ft_rwrapper'}));
			...
			if (cfg.fixedCols > 0) {
				...
				var tdct = 0;
				$j('tr', theadTrClone).first().find('th').filter( function () {
					tdct += this.colSpan;
				...
				lc.ft_rel_container
					.prepend($j(cfg.tableTmpl(), {'class': 'ft_rc ui-widget-header'})
					.append(theadTrClone));
				
				//an instance of fixed row column
				lc.ft_rc = $j('.ft_rc', lc.ft_rel_container);
				...
				$j.each ($this.find('tbody > tr'), function (idx, el) {
					var tr = $j(el).clone();
				...
				lc.ft_c.wrap($j('<div />', {'class': 'ft_cwrapper'}));

				var tw = 0;
				for (var i = 0; i < cfg.fixedCols; i++) {
					tw += $j(this.rows[0].cells[i]).outerWidth(true);
				}
				...
				for (var i = 0; i < this.rows.length; i++) {
					var ch = $j(this.rows[i]).outerHeight();
					var fch = $j(lc.ft_c[0].rows[i]).outerHeight(true);
			...
			lc.ft_wrapper.scroll(function () {
				if (cfg.fixedCols > 0) { 
					lc.ft_c.css('top', ($j(this).scrollTop()*-1));
				}
				lc.ft_r.css('left', ($j(this).scrollLeft()*-1));
			});
			...
			if (cfg.sort && sorttable && cfg.fixedCols == 0) {
				...
				$j('table', lc.ft_container).addClass('sorttable');
				
				sorttable.makeSortable(this);
				
				var $sortableTh = $j('.fx_sort_bg', lc.ft_rel_container);
