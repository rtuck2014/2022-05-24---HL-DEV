...
cfsapp.controller('cfsContainer', ['$cfsServices', '$scope', function($cfsServices, $scope) { 
	...
	$scope.navigate = function(url, openInNewWindow, defaultNavigationFunction) {
        ...
        if ($scope.pageContext && $scope.pageContext.isSalesforce1 == true && typeof sforce !== 'undefined' && sforce != null) {
...
cfsUtilsModule.factory('$cfsUtils', function($interval) {
    return {
        ...
        buildLoPropertyList : function(lo, labels) {
            ...
            if (lo.statusLabel) {
                propertyList.push({class: 'status', label: labels.lmscons__Assignments_Status+':', value: lo.statusLabel });
            }
            if (lo.keywords) {
                propertyList.push({class: 'keywords', label: labels.lmscons__EUI_Keywords+':', value: lo.keywords });
            }
            if (lo.language) {
                propertyList.push({class: 'language', label: labels.lmscons__ConsumerCatalogDetail_Lang+':', value: lo.language });
            }
            if (lo.trainingTypeLabel) {
                propertyList.push({class: 'type', label: labels.lmscons__SearchCatalog_Column_Type+':', value: lo.trainingTypeLabel });
            }
            if (lo.location) {
                propertyList.push({class: 'location', label: labels.Location+':', value: lo.location });
            }
            if (lo.instructor) {
                propertyList.push({class: 'instructor', label: labels.EUI_Instructor+':', value: lo.instructor });
            }
            if (lo.durationDisplay) {
                propertyList.push({class: 'duration', label: labels.lmscons__DURATION+':', value: lo.durationDisplay });
            }
            if (lo.passingScore) {
                propertyList.push({class: 'passingScore', label: labels.lmscons__EUI_Passing_Score+':', value: lo.passingScore });
            }
            if (lo.score) {
                propertyList.push({class: 'score', label: labels.lmscons__EUI_Your_Score+':', value: lo.score });
            }
            if (lo.dueDateDisplay && !lo.completionDateTimeDisplay) {
                propertyList.push({class: 'dueDate', label: labels.lmscons__DUE_DATE+':', value: lo.dueDateDisplay });
            }
            if (lo.completionDateTimeDisplay) {
                propertyList.push({class: 'completionDateTime', label: labels.lmscons__Assignments_Completed+':', value: lo.completionDateTimeDisplay });
            }
            return propertyList;
        }
    }
...
// lightbox switch of expanded calendar 
...
// service of multi-language locale format data
angular.module('cfsapp')
				...
				if (newValue) {
                    if ($scope.prereqs) {  //this is a refresh
						...
						for (i =0; i < $scope.prereqs.length; i++) {
                            var moduleProgressId = 'module-progress#prereq' + newValue.prerequisites[i].objectId;
                            $cfsUtils.refreshLO($scope.prereqs[i], newValue.prerequisites[i], $element.find(moduleProgressId));
                        }

                        if (newValue.prerequisites) {		        	
				        	for (i = 0; i < newValue.prerequisites.length; i++) {
				        		var item = newValue.prerequisites[i];
	...
	return {
		...
		controller: function($scope, $element) {
            ...
            $scope.$watch(dataSourceId, function(newValue, oldValue) {
                if (newValue) {
                    if ($scope.object) {  //this is a refresh, only update the necessary fields
                        ...
                        if (refreshInterval && (newValue.learningObject.status == 'Completed' || newValue.learningObject.status == 'Passed' || newValue.learningObject.status == 'Cancelled')) {
                            $interval.cancel(refreshInterval);
                            refreshInterval = null;
                        }
                    } else {
            ...
            $scope.$watch(enrollDataSourceId, function(newValue, oldValue) {
                $scope.enrollDisabled = false;
                if (newValue) {
                    var enrollresult = newValue;
                    $scope.object.enrollMessage = enrollresult.message;
                    if (enrollresult.isSuccess == true) {
                        $scope.object.isAlreadyAssigned = true;
                        $scope.object.enrollSuccessMessage = true;
                        $scope.fetchDataSource(refreshDataSourceName, refreshActionName, [$scope.object.objectId]);
                    }
                    else{
                        $scope.object.enrollErrorMessage = true;
                        $scope.object.isAlreadyAssigned = false;
            ...
            $scope.init = function() {
                var complete = $cfsUtils.getParameterByName('c');
                if (complete == 1) {
            ...
            $scope.openConfirmation = function(message) {
                $('.b-lightbox-confirm').fadeIn('fast');
            ...
            var markCompleteDataSourceName = markCompleteActionName = 'markComplete';
            var markCompleteDataSourceId = $scope.registerDataSource(markCompleteDataSourceName);
            $scope.$watch(markCompleteDataSourceId, function(newValue, oldValue) {
                // After mark complete, just refresh the lo details data source
                if (newValue) {
                    if (newValue.isSuccess) {
                        var moduleProgressId = 'module-progress#detail' + newValue.learningObject.objectId;
                        $cfsUtils.refreshLO($scope.object, newValue.learningObject, $element.find(moduleProgressId));                        
            ...
            $scope.markComplete = function(SFObjectId){
                $("#progressbar").show();
                $("#markCompleteBtn").hide();
                $scope.fetchDataSource(markCompleteDataSourceName, markCompleteActionName, [SFObjectId]);
            };

            var unenrollClassDataSourceName = unenrollClassActionName = 'unenrollClass';    
            var unenrollClassDataSourceId = $scope.registerDataSource(unenrollClassDataSourceName);        
            $scope.$watch(unenrollClassDataSourceId, function(newValue, oldValue) {
                $scope.unenrollDisabled = false;
                if (newValue) {
                    if (newValue.isSuccess == true) {
                        $scope.fetchDataSource(refreshDataSourceName, refreshActionName, [$scope.object.objectId]);
                    } else {
                        $scope.object.errorMessage = newValue.error;
                        $scope.openLoadingLightBox(newValue.error);
                        $("#progressbar").hide();
                    }
                }
            });

            $scope.unenrollClass = function(classId){
                $("#progressbar").show();
                 // Do not cache results as each enroll shoudl be a new event
                $scope.unenrollDisabled = true;
                $scope.object.enrollSuccessMessage = false;
                $scope.object.enrollErrorMessage = false;
                $scope.object.isAlreadyAssigned = false;
                $scope.fetchDataSource(unenrollClassDataSourceName, unenrollClassActionName, [classId]);
            }
		},
	}
}]);angular.module('cfsapp')
.directive('cfsLoDetailsRelatedlist', ['$cfsUtils', '$interval', function($cfsUtils, $interval) {
	return {
		...
		controller: function($scope, $element, $location) {
            ...
            $scope.init = function () {
                var joinSessionId = $cfsUtils.getParameterByName('j');
                if (joinSessionId != '') {  //this is only used by CFSLinks to directly launch WebEx/GTT for an enrolled session
                    for (i = 0; i < $scope.containerItems.length; i++) {
                        var item = $scope.containerItems[i];
                        if (item.objectId == joinSessionId) {
                            $scope.navigate(item.contextualActionLink, true, window.open);
                            return;        
                        }
                    }
                }
...
cfsRemoteActionModule.factory('$cfsServices', function() {
	...
	var enroll = function(namespacePrefix, objectId, trainingType, callback) {
        Visualforce.remoting.Manager.invokeAction(
    ...
    var markComplete = function(namespacePrefix, objectId, callback) {
        Visualforce.remoting.Manager.invokeAction(
            namespacePrefix + 'CFSAPI.markComplete',
            objectId,
            callback,
            {escape: false, buffer: false}
        );
    ...
    var unenrollClass = function(namespacePrefix, classId, callback) {
        Visualforce.remoting.Manager.invokeAction(
            namespacePrefix + 'CFSAPI.unenrollClass',
            classId,
            callback,
            {escape: false, buffer: false}
        );
    ...
    return {
        executeRemoteAction : function(actionName, args, callback) {
            ...
            if (actionName === 'searchCatalogs') {
                ...
                enroll(namespacePrefix, args[0], args[1], callback);
            }
            else if (actionName === 'markComplete') {
                markComplete(namespacePrefix, args[0], callback);
            }
            else if (actionName === 'unenrollClass') {
                unenrollClass(namespacePrefix, args[0], callback);
            }
            ...
            $scope.init = function() {              
                $(document).ready(function(){
                    ...
                    $('#training-type-multiselect').multiselect({ 
                        numberDisplayed: 1,
                        allSelectedText: $scope.labels['lmscons__SearchCatalog_All_Selected'],
                        nSelectedText: $scope.labels['lmscons__SearchCatalog_Selected'],
                        nonSelectedText: $scope.labels['lmscons__SearchCatalog_None_Selected']
                    });
                    ...
                    $('#catalog-multiselect').multiselect({
                        ...
                        buttonText: function(options, select) {
                            if (options.length === 0) {
                                return $scope.labels['lmscons__SearchCatalog_None_Selected'];
                            }
                            else if (options.length > 1) {
                                if ($scope.filter.catalogPicklist.length == options.length)
                                    return $scope.labels['lmscons__SearchCatalog_All_Selected'] + ' ('+options.length+')';
                                else
                                    return options.length + ' ' + $scope.labels['lmscons__SearchCatalog_Selected'];
                            }
                             else {
                                 var label = [];
