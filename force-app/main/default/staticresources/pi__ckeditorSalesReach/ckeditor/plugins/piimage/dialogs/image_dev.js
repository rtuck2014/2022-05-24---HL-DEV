...
(function() {
	var url = window.PardotBaseUrl,
		arr = url.split("/");
		domain = arr[0] + "//" + arr[2];
	var pdSelectBuild = function (data, obj, module) {
		var vals;
		...
		function highlightActive () {
			...
			if(urlField !== "") {
				var setH = dialog.getContentElement( 'info', 'txtHeight' ).getValue();
				var setW = dialog.getContentElement( 'info', 'txtWidth' ).getValue();
				var urlFieldVal = urlField.getValue();
				var imageSelect = dialog.getContentElement( 'info', 'uploadedImage' );
				var found = jQuery(imageSelect.getInputElement().$).find('option[value="'+urlFieldVal+'"]').length;
				if(found){
					obj.setValue(urlFieldVal);
					jQuery('.ImagePreviewLoader').hide();
				}
			}
				//search through dropdown and change it if value exists
		}
		...
		jQuery.ajax({
			url: domain + "/file/getSalesEdgeImageList",
			cache: false,
			success: function(d){									
				if(d) {
					obj.clear();
			...
			return {
						...
						children: [
							{
								...
								children: [
								{
									...
									onChange : function () {
										...
										if(urlField.getValue() !== newVal) {
											dialog.setValueOf('info','txtUrl',newVal);
										}
										urlField = urlField.getElement();
										if(newVal !== "") {
											urlField.hide();
										} else {
											urlField.show();
							...
							children: [
								...
								children: [
									...
									commit: function( type, element, internalCommit ) {
										...
										if ( type == IMAGE ) {
											if ( value ) {
												...
												element.setAttribute( 'width', parseInt(CKEDITOR.tools.cssLength( value )) );													
											}
											else {
									...
									commit: function( type, element, internalCommit ) {
										...
										if ( type == IMAGE ) {
											if ( value ) {
												element.setAttribute( 'height', parseInt(CKEDITOR.tools.cssLength( value )) );
									...
									commit: function( type, element, internalCommit ) {
										...
										if ( type == IMAGE || type == PREVIEW ) {
											if ( value ) {
												...
												var currentAlign = ( element.getAttribute( 'align' ) || '' ).toLowerCase();
												switch ( currentAlign ) {
				...
				},
				{
					...
					elements: [
						{
						type: 'html',
						id: 'uploadHtmlPreview',
						html: '<div id="uploadIframe" style="height:290px;"><iframe src="'+domain+'/file/salesEdgeUploader" style="height:100%;width:100%;"></iframe></div>',
						onShow: function(data) {
							jQuery('#uploadIframe iframe').attr('src', domain+'/file/salesEdgeUploader'); // reload iframe
						}
						},
						{
						id: 'dummy',
